...
BAUD = 460800
EXP = ROOT / "experiments" / "exp_audio_detector_1d_improved"
STREAM_EXP = ROOT / "experiments" / "exp_audio_detector_1d"
INPUT_LEN = 1024
...
def main(argv=None) -> int:
    ...
    model = LFFTDetector(n_filters=48).eval(); model.load_state_dict(ck["model"])

    z = np.load(STREAM_EXP / "stream_test.npz", allow_pickle=True)
